...
for i in range(N.size):
    [results, CT[i], CP[i], Uinf, Radius] = BEM.ExecuteBEM(N[i], plotter, mode = 'constant')
    
# Cosinus mode    
CT_cos = np.zeros(N.size)
CP_cos = np.zeros(N.size)
for i in range(N.size):
    [results, CT_cos[i], CP_cos[i], Uinf, Radius] = BEM.ExecuteBEM(N[i], plotter, mode = 'cosinus')
...
plt.figure(figsize=(12, 6))
plt.plot(N, CP_cos, 'r--', label='Cosine distribution')
plt.plot(N, CP, 'b--', label = 'Uniform distribution')
plt.grid()
plt.xlabel('N (number of annulus)', fontsize=20)
